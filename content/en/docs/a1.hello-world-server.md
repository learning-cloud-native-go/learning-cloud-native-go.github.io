...
So, let's move the `main.go` file to `cmd/api/main.go`.
...
> 💡 Btw, if you try to run `go run ./cmd/api` or `go build ./cmd/api` commands manually, the compiler will warn you about the missing `go.mod` file. We'll discuss this in the next section.
